...
public class PiecesAnnexesBean implements Serializable{
	...
	public void updatePieces(){
		if(this.pieceToConsult.getLibelle() != null){
			...
			//allPieces = pService.allPieces();
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Le document "+this.pieceToConsult.getLibelle()+" a été mise � jour!", null);
            FacesContext.getCurrentInstance().addMessage(null, message);
		}
		else{
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Erreur de la modification de la piéce "+this.pieceToConsult.getLibelle(), null);
            FacesContext.getCurrentInstance().addMessage(null, message);
		}
	}
